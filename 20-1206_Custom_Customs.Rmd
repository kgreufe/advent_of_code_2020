...
df %>% 
  ...
  map(.f = ~ mutate(., x = str_split(value, "", simplify = TRUE)) %>% # split characters into matrix
        .$x %>%  # extract matrix
        as.vector() %>% # change matrix to vector
        .[. != ""] %>%  # remove empty strings
        unique() %>% # remove duplicate characters
        sort() %>% # for aesthetics only
        str_c(collapse = "") %>% # condense to one string
        str_length() %>% # count length of string
        as_tibble() # convert to data.frame so we can bind rows below
      ) %>% 
  bind_rows() %>% 
  .$value %>% 
  sum()
...
Very complicated and probably an easier way to do this...
...
df %>% 
  ...
  map(.f = ~ mutate(.,  x = str_split(value, "", simplify = FALSE)) %>% 
        .$x %>% 
        reduce(.f = intersect) %>% 
        length()
  ) %>% 
